...
function EmailStateChanged() 
{
	...
	$main_content .= ' <center><h1>Bienvenido a  '.htmlspecialchars($config['server']['serverName']).' </h1></center>. 
						<center><BR><BR><h2>Para evitar el daño a nuestra base de datos y el spam dentro de ella es necesario: </h2> <h1> E-mail Real </h1> <h2> Para poder verificar la cuenta. De otro modo no podra creearla.</h2>
						<BR><BR></center>
...
if($action == "saveaccount")
{
	...
	if(empty($reg_form_errors))
	{
		...
		if(isset($config['site']['newaccount_vipdays']) && $config['site']['newaccount_vipdays'] > 0)
		{
			$reg_account->set("vipdays", $config['site']['newaccount_vipdays']);
			$reg_account->set("viplastday", time());
		}
		$reg_account->save();
